...
public class DBUtil {
	...
	public ArrayList<UserDTO> loginID(){
		SqlSession session = sqlSessionFactory.openSession();
		UserMapper mapper = session.getMapper(UserMapper.class);
		ArrayList<UserDTO> loginlist = mapper.loginID();
		
		
		return loginlist;
	}
	public ArrayList<UserDTO> loginPW(){
		SqlSession session = sqlSessionFactory.openSession();
		UserMapper mapper = session.getMapper(UserMapper.class);
		ArrayList<UserDTO> loginlist = mapper.loginPW();
	...
	public void updataUser(String name, String user_id) {
		SqlSession session = sqlSessionFactory.openSession();
		UserMapper mapper = session.getMapper(UserMapper.class);
